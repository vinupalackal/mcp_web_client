    ...
    *,
    ...
    if is_direct_fact:
        return (
            ...
            "2. Read ALL tool output in the conversation above — not just the summary table.\n"
            "3. Convert raw numbers into human-readable form "
            "(e.g. seconds → days/hours/minutes, kB → MB/GB, Hz → MHz/GHz).\n"
            "4. Structure your response with the following sections:\n\n"
            "   ## Top Result\n"
            "   State the primary answer (e.g. the #1 process) with ALL available fields "
            "from the tool output: PID, user/owner, RES memory, VIRT memory, memory %, "
            "CPU %, command/service name, and any other fields present.\n\n"
            "   ## Full List\n"
            "   Present EVERY entry returned by the tool in a markdown table. "
            "Columns should include all fields present in the raw output "
            "(e.g. Rank, PID, User, RES, VIRT, MEM%, CPU%, Command). "
            "Do NOT truncate or omit rows — show the complete tool output. "
            "Convert units (kB → MB/GB) in every row.\n\n"
            "   ## Observations  (omit if nothing notable)\n"
            "   Brief bullet notes on anything clearly abnormal — e.g. a single process "
            "dominating memory, unusually high counts, or failed entries. "
            "Do NOT speculate or add unsolicited maintenance advice.\n\n"
            "5. If the tool failed or returned partial data, state clearly what is missing "
            "and how that affects the answer.\n"
            "6. Use precise values; do not round aggressively or drop significant fields.\n"
    ...
    return (
        ...
        "INSTRUCTIONS:\n"
        "1. Do NOT call any more tools.\n"
        "2. Read ALL tool outputs in the conversation history — do not rely only on the "
        "summary table (it is truncated).\n"
        "3. Translate raw numbers into human-readable form "
        "(e.g. kB → MB/GB, seconds → days/hours/minutes, Hz → MHz/GHz).\n"
        "4. Produce a structured analytical response using the following sections:\n"
        "   ## Summary\n"
        "   One or two sentences stating the overall conclusion.\n\n"
        "   ## Key Findings\n"
        "   Bullet list of the most important values and observations from the tool "
        "output, with units converted and thresholds noted where applicable.\n\n"
        "   ## Analysis\n"
        "   Two to four sentences assessing whether each metric indicates a "
        "healthy / degraded / critical condition, cross-correlating related values "
        "where useful, and explaining any patterns or trends.\n\n"
        "   ## Anomalies & Risks  (omit section if none)\n"
        "   Bullet list of values, errors, or conditions that warrant attention, "
        "including their potential impact.\n\n"
        "   ## Recommendations  (omit section if none)\n"
        "   Numbered list of concrete next steps or mitigations, ordered by priority.\n"
        "5. If a tool failed, state clearly what could not be determined and note "
        "whether the gap affects the overall assessment.\n"
        "6. Use precise values from the tool output; do not round aggressively or "
        "omit detail. Prefer a thorough answer over a brief one.\n"
        f"\nUser request: {current_user_message}"
    )
